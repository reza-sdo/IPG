...
savedCardsHandler();
showSavedCardBoxHandler();

addDotIn3Digits();

getBtcPrice(); // 100request in month  , To speed up the work

cryptoHandler();

const randNum8digits = () => {
...
// To place the saved cards in a special place , These cards come from local storage
...
//ex year handler in card show , add 0 to 1 digits
...
//ex month handler in card show , add 0 to 1 digits , month most smaller than 12
...
// cvv2 should be more than 3 digits
...
// to rotate btn for shows save cards and show save cards btn
...
// to clear card number input and rest card show number(aside)
...
// change request code btn to disable
...
/////////////////////////// checkers to request btn and final pay btn
...
// to send sms   note : To receive SMS, you must not turn off the advertising SMS of your SIM card!!
...
function sendSms() {
   ...
   const code = randNum8digits();
   console.log(code); // if you don't receive the sms
...
function finalHandeler() {
   ...
   trNo.innerText = `شماره پیگیری :  ${randNum8digits()}`;
   refNo.innerText = `شماره مرجع :  ${randNum8digits()}`;
}
function finalBtn() {
   finalContinueBtn.addEventListener("click", backToShopOk);
}

// for show tehrans wether
function wether() {
...
// If the payment was not successful and cancel btn
